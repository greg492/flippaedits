...
def _on_beats_detected(self, beats: np.ndarray, onset_env: np.ndarray, tempo: float) -> None:
    ...
    # Calculate beat intensities for visualization
...
def _calculate_beat_intensities(self, beats: np.ndarray, onset_env: np.ndarray, sr: int = 22050, hop_length: int = 512) -> np.ndarray:
...
```
  ...
  <done>
MainWindow has music_panel widget, beat detection workflow triggers on music import, beat snapper applies to timestamp marking
  </done>
</task>
...
3. Human verification confirms end-to-end functionality
</verification>
